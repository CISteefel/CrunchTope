...
WRITE(iunit2,*)
WRITE(iunit2,*) ' ************************** CRUNCH ******************************'
WRITE(iunit2,*) ' ************************ VERSION 1.0 ***************************'
WRITE(iunit2,*)
WRITE(iunit2,*) '                     Author:  C.I. STEEFEL '
WRITE(iunit2,*) ' COPYRIGHT (c) 2000. The Regents of the University of California. '
WRITE(iunit2,*) '                      All Rights Reserved '
WRITE(iunit2,*)
WRITE(iunit2,*) ' ****************************************************************'

WRITE(iunit2,*) ' This work was produced at the University of California, Lawrence '
WRITE(iunit2,*) ' Livermore National Laboratory (UC LLNL) under contract no. W-7405-ENG-48 '
WRITE(iunit2,*) ' (Contract 48) between the U.S. Department of Energy (DOE) and The Regents '
WRITE(iunit2,*) ' of the University of California (University) for the operation of UC LLNL. '
WRITE(iunit2,*) ' Copyright is reserved to the University for purposes of controlled dissem- '
WRITE(iunit2,*) ' ination, commercialization through formal licensing, or other disposition '
WRITE(iunit2,*) ' under terms of Contract 48; DOE policies, regulations and orders; and U.S. '
WRITE(iunit2,*) ' statutes. The rights of the Federal Government are reserved under Contract '
WRITE(iunit2,*) ' 48 subject to the restrictions agreed upon by the DOE and University as '
WRITE(iunit2,*) ' allowed under DOE Acquisition Letter 97-1. '
WRITE(iunit2,*)

WRITE(iunit2,*) '                               DISCLAIMER                            '

WRITE(iunit2,*) ' This work was prepared as an account of work sponsored by an agency of the'
WRITE(iunit2,*) ' United States Government. Neither the United States Government nor the '
WRITE(iunit2,*) ' University of California nor any of their employees, makes any warranty, '
WRITE(iunit2,*) ' express or implied, or assumes any liability or responsibility for the '
WRITE(iunit2,*) ' accuracy, completeness, or usefulness of any information, apparatus, product,'
WRITE(iunit2,*) ' or process disclosed, or represents that its use would not infringe '
WRITE(iunit2,*) ' privately-owned rights.  Reference herein to any specific commercial products,'
WRITE(iunit2,*) ' process, or service by trade name, trademark, manufacturer or otherwise does '
WRITE(iunit2,*) ' not necessarily constitute or imply its endorsement, recommendation, or '
WRITE(iunit2,*) ' favoring by the United States Government or the University of California. '
WRITE(iunit2,*) ' The views and opinions of authors expressed herein do not necessarily state '
WRITE(iunit2,*) ' or reflect those of the United States Government or the University of '
WRITE(iunit2,*) ' California, and shall not be used for advertising or product endorsement '
WRITE(iunit2,*) ' purposes. '
WRITE(iunit2,*) ' ****************************************************************'
...
!  Allocate "Y" arrays that depend on NY here
  ...
  y(1) = 0.5*dyy(1)
  DO jy = 2,ny
    y(jy) = y(jy-1) + 0.5*dyy(jy-1) + 0.5*dyy(jy)
  END DO
...
IF(nAqueousFluxSeriesFile /= 0) jzAqueousFluxSeries_hi(1:nAqueousFluxSeriesFile) = workint1(1:nAqueousFluxSeriesFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jxFluxWeightedConcentration_lo(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jyFluxWeightedConcentration_lo(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jzFluxWeightedConcentration_lo(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jxFluxWeightedConcentration_hi(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jyFluxWeightedConcentration_hi(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
IF(nFluxWeightedConcentrationFile /= 0) jzFluxWeightedConcentration_hi(1:nFluxWeightedConcentrationFile) = workint1(1:nFluxWeightedConcentrationFile)
DEALLOCATE(workint1)
...
!!        WRITE(*,*)
!!        WRITE(*,*) ' For flow problems, use X and Y coordinates first (Z for 3rd dimension)'
!!        WRITE(*,*) ' Y coordinate may be set to coincide with gravity vector--set "gravity  90.0  0.0  90.0"'
!!        WRITE(*,*)
!!        STOP
!!      END IF
