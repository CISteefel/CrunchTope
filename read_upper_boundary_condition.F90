SUBROUTINE read_upper_boundary_condition(nout, BC_type, BC_file, value, lfile, constant_BC, time_length)
! This subroutine reads the upper boundary condition for the Richards equation
USE crunchtype
USE CrunchFunctions
USE params
USE strings
...
IF(ls /= 0) THEN
  ...
  IF (ssch == 'upper_bc_type') THEN
      ...
      ELSE
        ...
        CASE ('constant_dirichlet', 'constant_neumann', 'constant_flux') bc_case
          ...
          IF(ls /= 0) THEN
            ...
            ELSE
            WRITE(*,*)
            WRITE(*,*) ' Must provide a numerical value for the constant upper boundary condition. '
            WRITE(*,*)
            READ(*,*)
            STOP  
            ENDIF
          ELSE
            WRITE(*,*)
            WRITE(*,*) ' No information provided on the constant upper boundary condition. '
            WRITE(*,*)
            READ(*,*)
            STOP  
          ENDIF
        CASE ('variable_dirichlet', 'variable_neumann', 'variable_flux') bc_case
          ...
          IF(ls /= 0) THEN
            ...
            IF (res == 'n') THEN
              WRITE(*,*)
              WRITE(*,*) ' Must provide a filename for the variable upper boundary condition. '
              WRITE(*,*)
              READ(*,*)
            ELSE
              ! obtain the file name for the variable boundary condition
              ...
              ELSE
                WRITE(*,*)
                WRITE(*,*) ' Must provide a length for time series for the upper boundary condition. '
                WRITE(*,*)
                READ(*,*)
                STOP  
              ENDIF
          ...
          ELSE
            WRITE(*,*)
            WRITE(*,*) ' No information provided on the filename for the variable upper boundary condition. '
            WRITE(*,*)
            READ(*,*)
            STOP  
          ENDIF
    ...
    ELSE
      WRITE(*,*)
      WRITE(*,*) ' No information provided on the upper boundary condition. '
      WRITE(*,*)
      READ(*,*)
      STOP
    END IF
...
1000 RETURN
END SUBROUTINE read_upper_boundary_condition