...
TYPE :: RichardsBase ! store information used in the Richards solver, including discretization information
  CHARACTER (LEN=264) :: spatial_domain ! shape of the spatial domain (only used in 2D Richards)
...
TYPE :: RichardsOptions ! store options for Richards solver
  ...
  LOGICAL(LGT) :: vg_is_n = .FALSE. ! True if the input to vg_n is the n parameter in the van Genuchten model, otherwise, the input value is interpreted as the m parameter
  LOGICAL(LGT) :: psi_is_head = .FALSE. ! True if the primary variable psi in the Richards equation is pressure head [L] or not. If false, the input values for the initial and boundary conditions, and vg_alpha are interpreted as in terms of pressure [Pa].  
  LOGICAL(LGT) :: theta_s_is_porosity = .FALSE. ! True if the input to theta_s is the same as the porosity
